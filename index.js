...
function run () {
  ...
  if (isMacOS) {
    job = execCommand(getCommand('', port, dir, true))
  } else {
...
function getCommand (phpDir, port, dir, isMacOS) {
  // prepare flag for document root directory
  var dirCommand = dir ? ' -t ' + dir : ''

  // compound all together
  return isMacOS ? 'php -S localhost:' + port + dirCommand : phpDir + '/php.exe -S localhost:' + port + dirCommand
}
